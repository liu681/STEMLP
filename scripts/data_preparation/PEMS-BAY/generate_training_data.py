...
def generate_data(args: argparse.Namespace):
    ...
    if add_time_of_tp2:
        # numerical time_of_day
        tod = (
            df.index.values - df.index.values.astype("datetime64[D]")) % np.timedelta64(12,"h") / np.timedelta64(12, "h")
        tod_tiled = np.tile(tod, [1, n, 1]).transpose((2, 1, 0))
        print(tod_tiled)
        feature_list.append(tod_tiled)
    if add_time_of_tp3:
        # numerical time_of_day
        tod = (
                df.index.values - df.index.values.astype("datetime64[D]")) % np.timedelta64(8,"h") / np.timedelta64(8, "h")
        tod_tiled = np.tile(tod, [1, n, 1]).transpose((2, 1, 0))
        print(tod_tiled)
        feature_list.append(tod_tiled)
